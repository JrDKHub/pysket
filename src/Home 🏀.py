...
st.sidebar.image("src/assets/img/pysket.png", width=300)
st.sidebar.markdown(
    f"<h1 style='text-align: center;'>PYSKET</h1>", unsafe_allow_html=True
)
...
away = team2.selectbox(
    label="Choisir l'équipe à l'extérieur", options=all_teams_names, key="Team2"
)
...
versus.image(
    "https://static.vecteezy.com/ti/vecteur-libre/p3/13212902-signe-vs-ou-versus-symbole-de-competition-vectoriel.jpg"
)
if versus.button("Simuler la rencontre", type="primary"):
    ...
    team1.markdown(
        f"<h1 style='text-align: center; color:{code1};'>{round(score_home)}</h1>",
        unsafe_allow_html=True,
    )
    team2.markdown(
        f"<h1 style='text-align: center;color:{code2};'>{round(score_away)}</h1>",
        unsafe_allow_html=True,
    )
